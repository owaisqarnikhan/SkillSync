...
export default function Sidebar() {
  ...
  // Emit sidebar toggle event for main layout
  const toggleSidebar = () => {
    const newCollapsed = !isCollapsed;
    setIsCollapsed(newCollapsed);
    
    // Emit custom event for App.tsx to listen
  ...
  const SidebarContent = ({ isMobile = false }: { isMobile?: boolean }) => (
    <div className="flex flex-col h-full">
      ...
      {/* Navigation */}
      <ScrollArea className={`flex-1 py-4 ${!isMobile && isCollapsed ? 'px-1' : 'px-3'}`}>
        <TooltipProvider>
  ...
  return (
    <>
      {/* Desktop Sidebar */}
      <div className={`hidden md:flex md:flex-col md:fixed md:inset-y-0 z-50 transition-all duration-300 ${isCollapsed ? 'md:w-16' : 'md:w-64'}`}>
